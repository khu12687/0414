class Megaman extends GameObject{
    constructor(objectId,container,src,width,heigh,velX,velY,x,y){
        ...
        //메가맨의 이미지는 sprite 이기 때문에 div안에서 마스크처리 되어야 한다..
        // 따라서 이미지의 크기와 div가 같지 않다!!
        //div를 넘는 이미지에 대해서는 않보이게
    ...
    tick(){
        ...
        this.y+=this.velY; //방향은 y축으로 함으로써 중력가속도표현

        //벽돌위에 서 있기
        for(var i=0;i<objectManager.objectArray.length;i++){
            // 배열에 들어있는 요소가 섞여있기때문에, 객체에 심어놓은
            // objectId를 이용하여 구분지은 후, 벽돌위에 서있게 처리
            ...
            if(gameObject.objectId=="Block"){
               ...
               if(onBlock){ //벽돌과 마주치면... 메가멘의 위치는 벽돌보다
                    ...
                    this.y=gameObject.y-this.heigh;
                    this.velY=0;
                    this.y=gameObject.y-this.heigh;
               }
            }
